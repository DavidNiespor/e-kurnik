    ...
    def admin_farm_assign():
        ...
        if request.method == "POST":
            ...
            if action == "add" and uid and farm_id:
                try:
                    db.execute(
                        "INSERT OR IGNORE INTO uzytkownicy_gospodarstwa(uzytkownik_id,gospodarstwo_id,rola) VALUES(?,?,?)",
                        (uid, farm_id, rola)
                    )
                    db.commit()
                    flash("Przypisano farmę do użytkownika.")
        ...
        users  = db.execute("SELECT id,login,email FROM uzytkownicy WHERE rola!='superadmin' ORDER BY login").fetchall()
        ...
        u_opt = "".join('<option value="' + str(u["id"]) + '">' + u["login"] + ' (' + (u["email"] or "") + ')</option>' for u in users)
        f_opt = "".join('<option value="' + str(f["id"]) + '">' + f["nazwa"] + '</option>' for f in farms)
        ...
        w_assign = "".join(
            ...
            '<button class="btn br bsm" onclick="return confirm(\'Usunąć przypisanie?\')">Usuń</button>'
            '</form>'
            '</td></tr>'
            for a in assign
        )
        ...
        html = (
            ...
            '<div><label>Farma</label><select name="farm_id">' + f_opt + '</select></div>'
            '<div><label>Rola</label><select name="rola">'
            '<option value="owner">owner (właściciel)</option>'
            '<option value="member" selected>member (pracownik)</option>'
            ...
            '<tbody>' + (w_assign or '<tr><td colspan=4 style="color:#888;text-align:center;padding:16px">Brak przypisań</td></tr>') + '</tbody></table></div>'
            '<p style="font-size:12px;color:#888;margin-top:8px">'
            '<a href="/admin" style="color:#534AB7">← Wróć do panelu admina</a></p>'
        )
        return R(html, "admin")

    # ════════════════════════════════════════════════════════════════════════
    # 6. WYDATKI — autocomplete ze składników magazynu
    # ════════════════════════════════════════════════════════════════════════
